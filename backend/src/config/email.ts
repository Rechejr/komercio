...
// EMAIL_FROM (ej. "Ventrix <noreply@ventrix.lat>") una vez el dominio esté
...
export const emailService = {
  async sendVerification(to: string, name: string, token: string) {
    ...
    await send(to, 'Verifica tu cuenta en Ventrix', verificationTemplate(name, url), 'Email verification send error');
  },
      ...
      <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
        ...
        <tr><td style="padding:40px 32px;">
          ...
          <div style="text-align:center;margin:0 0 32px;">
            <a href="${url}" style="background:#0DA06A;color:#fff;text-decoration:none;padding:14px 32px;border-radius:6px;font-size:15px;font-weight:bold;display:inline-block;">Verificar mi cuenta</a>
          </div>
