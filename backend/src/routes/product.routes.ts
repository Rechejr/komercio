...
router.post('/',
  ...
  [
    ...
    body('stock').optional({ checkFalsy: true }).isFloat({ min: 0 }).withMessage('Stock inválido'),
    body('wholesalePrice').optional({ checkFalsy: true }).isFloat({ min: 0 }).withMessage('Precio mayorista inválido'),
    body('minStock').optional({ checkFalsy: true }).isFloat({ min: 0 }).withMessage('Cantidad mínima inválida'),
    body('taxRate').optional({ checkFalsy: true }).isFloat({ min: 0, max: 100 }).withMessage('IVA inválido'),
  ],
  validate,
