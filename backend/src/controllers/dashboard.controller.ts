...
import { bogotaDayStart, bogotaMonthStart } from '../utils/bogotaTime';
import { getOrGenerateAiSummary } from '../services/aiSummary.service';
...
export const dashboardController = {
  ...
  async getAiSummary(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const businessId = req.user!.businessId!;
      const result = await getOrGenerateAiSummary(businessId);
      if (!result) {
        return success(res, { summary: null, createdAt: null });
      }
      return success(res, result);
    } catch (err) {
      next(err);
