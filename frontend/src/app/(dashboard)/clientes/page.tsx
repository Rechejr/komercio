...
import {
  Plus, Search, Edit, Trash2, Users, X, Loader2, AlertCircle, ChevronRight,
...
export default function ClientesPage() {
  ...
  const previewMutation = useMutation({
    mutationFn: (file: File) => {
      ...
      fd.append('file', file);
      return api.post('/customers/import?dryRun=true', fd, {
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {isFree ? (
          ...
          <a
            href={`${process.env.NEXT_PUBLIC_API_URL}/customers/import-template`}
        ...
        <input ref={fileInputRef} type="file" aria-label="Seleccionar archivo Excel para importar clientes"
                ...
                >
                  ...
                  <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-1.5 justify-end">
                      <button type="button" aria-label="Editar cliente" onClick={() => openEdit(c)}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/[0.08] rounded-2xl shadow-modal w-full max-w-lg flex flex-col max-h-[90vh] animate-scale-in">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/[0.06]">
              <h2 className="text-[15px] font-semibold text-slate-800 dark:text-white flex items-center gap-2">
                <FileUp size={16} className="text-emerald-500" /> Vista previa — Clientes
              </h2>
            ...
            {previewMutation.isPending ? (
              ...
              <div className="p-6 space-y-5 overflow-y-auto min-h-0">
                <p className="text-[13px] text-slate-600 dark:text-slate-400">
                  ...
                  cliente{previewData.total !== 1 ? 's' : ''} en el archivo
                </p>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/[0.08] rounded-2xl shadow-modal w-full max-w-lg max-h-[90vh] flex flex-col animate-scale-in">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/[0.06] flex-shrink-0 bg-white dark:bg-slate-900">
              ...
              <div className="flex items-center gap-1.5">
                <button type="button" aria-label="Editar" onClick={() => { setSelected(null); openEdit(detail); }}
                  className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition">
                  <Edit size={14} />
                </button>
                {canDelete && (
            ...
            <div className="p-6 space-y-5 overflow-y-auto min-h-0">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 rounded-xl p-3.5 text-center">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-emerald-500 dark:text-emerald-400 mb-1">Total compras</p>
                  <p className="text-[20px] font-bold text-emerald-700 dark:text-emerald-300 tabular-nums">{detail._count?.sales || 0}</p>
                </div>
              </div>
                    ...
                    <div className="divide-y divide-slate-50 dark:divide-white/[0.04]">
                      {detail.sales.map((s: any) => (
                        <div key={s.id} className="flex items-center justify-between px-3 py-2.5 text-[13px]">
                          <span className="font-mono text-[12px] text-emerald-600 dark:text-emerald-400">{s.invoiceNumber}</span>
