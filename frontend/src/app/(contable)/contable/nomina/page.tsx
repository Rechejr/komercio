...
export default function NominaPage() {
  ...
  const [delTarget, setDelTarget] = useState<VencNomina | null>(null);

  // Solo Nómina: el backend filtra por obligación (listVencimientos acepta ?obligacion).
  const { data: items = [], isLoading } = useQuery<VencNomina[]>({
    queryKey: ['contable-nomina', search],
    queryFn: () =>
      api.get(`/contable/vencimientos?obligacion=nomina&search=${encodeURIComponent(search)}`).then((r) => r.data.data),
  });

  const invalidar = () => {
    qc.invalidateQueries({ queryKey: ['contable-nomina'] });
  ...
  return (
    <div className="space-y-4 animate-fade-up">
      ...
      <div className="flex items-start gap-2.5 rounded-xl border border-emerald-200 dark:border-emerald-800/60 bg-emerald-50/60 dark:bg-emerald-900/10 px-4 py-3">
        <Receipt size={18} className="text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-0.5" />
        <p className="text-[13px] text-slate-600 dark:text-slate-300 leading-relaxed">
          Nómina electrónica. Cada mes se reporta dentro de los primeros <strong>10 días hábiles del mes siguiente</strong>;
          la fecha de vencimiento se calcula sola (descontando festivos) — no hay que buscarla.
        </p>
      </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/[0.06]">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-slate-400 dark:text-slate-500">
                  <Receipt size={30} className="mx-auto mb-2" strokeWidth={1.5} />
                  <p className="text-sm">
                    {search ? 'No hay nómina que coincida con la búsqueda.' : 'Aún no has generado nómina. Usa “Generar nómina”.'}
                  </p>
                </td></tr>
              ) : (
                items.map((v) => (
      ...
      {modalOpen && <GenerarNominaModal onClose={() => setModalOpen(false)} onDone={invalidar} />}

      <ConfirmDialog
        open={!!delTarget}
        onOpenChange={(o) => !o && setDelTarget(null)}
        title="¿Eliminar este mes de nómina?"
