...
function GenerarExogenaModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const { data: periodos = [], isFetching: cargando } = useQuery<{ periodo: string; fecha: string }[]>({
    queryKey: ['contable-periodos', cliente?.id, 'exogena'],
    queryFn: () => api.get(`/contable/calendario/periodos?taxClientId=${cliente!.id}&obligacion=exogena`).then((r) => r.data.data),
    enabled: !!cliente,
  });

  const generarMut = useMutation({
    mutationFn: () => api.post('/contable/vencimientos/generar', { taxClientId: cliente!.id, obligacion: 'exogena' }).then((r) => r.data),
    onSuccess: (res: any) => { onDone(); toast.success(res?.message || 'Exógena generada'); onClose(); },
    onError: (e: any) => toast.error(e.response?.data?.error || 'No se pudo generar la exógena'),
  });
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-modal w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col animate-scale-in" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-100 dark:border-white/[0.06]">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Generar exógena</h2>
        ...
        <div className="px-6 py-5 overflow-y-auto flex-1 space-y-4">
          ...
          {cliente && (
            cargando ? (
              <div className="flex items-center gap-2 text-sm text-slate-400 py-2"><Loader2 size={14} className="animate-spin" /> Calculando fecha…</div>
            ) : periodos.length === 0 ? (
              <p className="text-[13px] text-amber-600 dark:text-amber-400">No hay calendario de exógena cargado para este año.</p>
            ) : (
              <div>
                <div className="flex items-center gap-1.5 text-[11px] text-emerald-600 dark:text-emerald-400 mb-1.5">
                  <Sparkles size={11} /> Se registrará este vencimiento (fecha automática según el documento)
                </div>
                ...
                <p className="text-[11px] text-slate-400 mt-1.5">Si ya existe, no se duplica.</p>
              </div>
            )
          )}
        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 dark:border-white/[0.06] flex gap-2">
          ...
          <button
            ...
            disabled={!cliente || cargando || periodos.length === 0 || generarMut.isPending}
